...
export interface AINovelSourceChapter {
  number: number;
  title: string;
  summary: string;
}

export interface AINovelCharacter {
  id: string;
  name: string;
  role: string;
  description: string;
...
export interface AINovelDialogueLine {
  character: string;
  emotion: string;
  line: string;
}
...
export interface AINovelScene {
  ...
  summary: string;
  characters: string[];
  dialogues: AINovelDialogueLine[];
  actions: string[];
}

export interface AINovelScreenplay {
  title: string;
  logline: string;
  language: string;
  target_format: string;
  source_chapters: AINovelSourceChapter[];
  characters: AINovelCharacter[];
  scenes: AINovelScene[];
}

export interface AINovelProjectPayload {
  title: string;
  source_text: string;
  target_format: string;
  language: string;
...
export interface AINovelProjectResponse {
  success: boolean;
  name: string;
  project: ProjectData;
  screenplay: AINovelScreenplay;
  yaml_text: string;
...
function handleUnauthorized(response: Response): void {
  ...
  clearToken();
  throw new Error("请求未授权，请检查后端 AUTH_ENABLED 配置或 API 凭据。");
}
...
class API {
  ...
  static async createNovelProject(
